...
class WatchSetting: Identifiable, Codable {
  ...
  var videoDimensionValue: Int? = 1920
  ...
  var subfolderProcessing: SubfolderProcessing? = SubfolderProcessing.none
  var subfolderProcessingLimit: Int? = 1
}
...
struct WatchSettingView: View {
  ...
  @State private var subfolderProcessing: SubfolderProcessing = .none
  @State private var subfolderProcessingLimit = 1
  ...
  var body: some View {
    Form {
      ...
      Section {
        ...
        .onChange(of: subfolderProcessing, perform: { newValue in
          setting.subfolderProcessing = newValue
          updateSetting()
        })
        .onChange(of: subfolderProcessingLimit, perform: { newValue in
          setting.subfolderProcessingLimit = newValue
          ...
          HStack {
            ...
            .popover(isPresented: $showOutputFileNameFormatPopover) {
              VStack(alignment: .leading) {
                ...
                Text("{resoluion} - Resolution of the output file")
              }
              .padding()
            }
          }
    ...
    .task {
      ...
      imageFormat = setting.imageFormat ?? .same
      ...
      subfolderProcessing = setting.subfolderProcessing ?? .none
      subfolderProcessingLimit = setting.subfolderProcessingLimit ?? 1
    }
  }
  ...
  var formHeight: CGFloat {
    ...
    if subfolderProcessing == .none || subfolderProcessing == .all {
      result += 40
    } else {
      result += 85
    }
  ...
  func onMaxDepthSubmittion() {
    if let limit = Int(subfolderProcessingLimitText), limit > 0 {
      ...
      setting.subfolderProcessingLimit = subfolderProcessingLimit
      updateSetting()
    } else {
      let alert = NSAlert()
      alert.messageText = "Invalid value"
