...
enum DropZoneMode: String, CaseIterable, Codable {
  case compress
  case compressAndOpen
  ...
  var body: some View {
    Form {
      Section {
        ...
        if dropZoneEnabled {
          Picker("Drop Zone mode", selection: $dropZoneMode) {
            ForEach(DropZoneMode.allCases, id: \.self) { mode in
      ...
      if dropZoneEnabled {
        if dropZoneCompressionSettingsType == .preset {
          Section {
            Picker("Preset", selection: $dropZonePresetId) {
              Text("None").tag("")
            ...
            .pickerStyle(.menu)
            if let preset = PresetManager.shared.preset(for: dropZonePresetId) {
              Text(preset.summary)
                .font(.caption)
