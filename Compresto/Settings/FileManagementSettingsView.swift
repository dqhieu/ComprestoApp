...
enum SubfolderProcessing: String, CaseIterable, Codable {
  case none
  case custom
  case all
...
struct FileManagementSettingsView: View {
  ...
  @AppStorage("subfolderProcessing") var subfolderProcessing: SubfolderProcessing = .all
  ...
  var body: some View {
    Form {
      ...
      VStack(alignment: .leading, spacing: 4) {
        ...
        case .replace:
          Text("Hold Option key to append files.")
            .font(.caption)
            .foregroundStyle(.secondary)
        case .append:
          Text("Hold Option key to replace files.")
            .font(.caption)
            .foregroundStyle(.secondary)
        }
      }
  ...
  func getSampleOutputFileName1() -> String {
    let format = currentOutputFileNameFormat
      ...
      .replacingOccurrences(of: "{resolution}", with: "FullHD")
