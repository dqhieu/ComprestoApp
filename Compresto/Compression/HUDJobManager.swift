...
class HUDJobManager: ObservableObject {
  ...
  var isAnyJobAIRenaming: Bool {
    jobs.contains { $0.isAIRenaming }
  }
  ...
  func compress(job: Job, isRetrying: Bool = false) async -> String? {
    ...
    currentProcess = nil
    if error == nil,
       case .video(let videoQuality, let videoDimension, let videoDimensionValue, let videoFormat, let targetFileSize, let hasAudio, let removeAudio, let preserveTransparency, let startTime, let endTime) = job.outputType,
  ...
  func compress() async -> [Job] {
    ...
    while let job = jobs[safe: i] {
      ...
      if job.isImage && preserveColorProfile {
        try? JobManager.shared.copyICCProfile(job: job)
      }
      ...
      #if !SETAPP
      if AIRenamingManager.shared.aiRenamingEnabled && job.error == nil {
    ...
    await MainActor.run {
      ...
      JobManager.shared.putComputerToSleepIfNeeded()
    }
