...
class Watcher {
  ...
  func start(settings: [WatchSetting]) {
    ...
    for setting in settings {
      var foldersToWatch: [String]
      if let subfolderProcessing = setting.subfolderProcessing {
        var maxDepth = 1
        switch subfolderProcessing {
        case .none:
          maxDepth = 1
        case .custom:
          maxDepth = setting.subfolderProcessingLimit ?? 1
        case .all:
          maxDepth = 1_000
        }
        foldersToWatch = getFoldersRecursively(url: URL(fileURLWithPath: setting.folder), maxDepth: maxDepth).map { $0.path }
      } else {
        foldersToWatch = [setting.folder]
      }
      for folder in foldersToWatch {
        let watcher = FolderMonitor(folderURL: URL(fileURLWithPath: folder, isDirectory: true))
        watcher.startMonitoring { [weak self] path in
          guard let self else { return }
          handleChangeIn(path: path, setting: setting)
        }
  ...
  func handleChangeIn(path: String, setting: WatchSetting) {
    ...
    if !currentOutputFiles.contains(where: { $0 == path }),
       let updatedSetting = watchSettings.first(where: { $0.id == setting.id }) {
      Task {
        let inputFileURL = URL(fileURLWithPath: path)
        let hasAudio = await fileHasAudio(url: inputFileURL)
        ...
        let outputType: OutputType? = {
          ...
          case .video:
            return .video(
              ...
              videoDimensionValue: updatedSetting.videoDimensionValue ?? 1920,
...
class FolderMonitor {
  ...
  func startMonitoring(handler: @escaping (String) -> Void) {
    ...
    folderMonitorSource?.setEventHandler { [weak self] in
      ...
      do {
        let currentFileList = (try FileManager.default.contentsOfDirectory(atPath: this.monitoredFolderURL.path)).filter { !$0.hasPrefix(".") && !$0.hasSuffix(".crdownload") }
  ...
  func shouldNotifyNewFiles(currentFiles: [String], oldFiles: [String]) -> Bool {
    ...
    for file in removedFiles {
      if removedFiles.contains(where: { $0 + ".crdownload" == file }) {
        return true
      }
      if removedFiles.contains(where: { $0.hasPrefix(".")}) {
        return true
      }
    }
