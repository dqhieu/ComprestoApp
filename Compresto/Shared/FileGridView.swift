...
struct FileInfoOverlay: View {
  ...
  var body: some View {
    ...
    .task {
      dimensionsText = await loadDimensions()
    }
  }

  private func loadDimensions() async -> String? {
...
struct FileGridView: View {
  ...
  func adjustColumns(to size: CGSize)  {
    if inputFiles.count == 1 || size.width <= 600 {
      columns = [
        GridItem(.flexible()),
      ]
  ...
  func getAvailableSize(size: CGSize) -> CGFloat {
    return (size.width - sidePadding * 2 - CGFloat(columns.count - 1) * 8) / CGFloat(columns.count)
  }
